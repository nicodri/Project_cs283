...
In order to classify a whale, we split the work in two parts. The first part consists of localizing the nose, its parts and extracting a feature vector from it for additional precision (optional). The second part deals with the actual classification which shall be done using a part-based one-vs-one features (POOF) approach. 
\section*{Part I: Localizing the nose and parts of it}
As mentioned in the introduction callosities and shape / size of a head are main features to distinguish a whale from the others. In order to localize the nose and parts of it, an approach using HOG features as described in \url{http://lear.inrialpes.fr/people/triggs/pubs/Dalal-cvpr05.pdf} shall be implemented.   
\section*{Part II: Part-based one-vs-one features}
<TODO>
